...
class MeteorShower(LightEffect):
    ...
    def update(self):
        ...
        move_distance = self.speed.get() * self.frame_length * 1000
        move_vector = [
            -math.sin(math.radians(self.angle.get())) * move_distance,
            -math.cos(math.radians(self.angle.get())) * move_distance,
            0
        ]
        self.spawn_delay = (self.amount.get() / self.speed.get()) / 10
        ...
        for meteor in self.meteors:
            ...
            for idx, coord in enumerate(self.coords):
                ...
                l = coord
                dist = abs(l[0]**2 * (t[1] - m[1]) + l[1]**2 * (m[0] - t[0]) - (m[0] * (t[1] - m[1]) + m[1] * (m[0] - t[0]))) / math.sqrt(l[0]**2 + l[1]**2)
                if dist < self.thickness.get():
                    # calculate brightness based on ratio of distance from M to T
                    ratio = 1 - (math.sqrt((l[0] - m[0])**2 + (l[1] - m[1])**2) - dist**2) / (math.sqrt((t[0] - m[0])**2 + (t[1] - m[1])**2))
                    col = self.color.get()
                    self.renderer[idx] = [
                        int(col[0] * ratio),
                        int(col[1] * ratio),
