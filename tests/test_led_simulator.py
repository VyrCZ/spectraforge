...
def test_led_simulator_receive_loop_handles_malformed_json():
    ...
    sim.sock = MagicMock()
    # The receive loop decodes from the start of the buffer; it cannot resync
    # past leading garbage so it exits cleanly on the next empty recv.
    sim.sock.recv.side_effect = [b"not-json-garbage", b""]

    sim._receive_loop()

    # State must be unchanged — garbage data must not corrupt colours
    assert sim.colors == [[0, 0, 0]]
